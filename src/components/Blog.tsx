...
interface TermItem {
  ...
  heading?:string;
  image?: string;
  description: React.ReactNode; // Assuming description can contain JSX
}
      ...
      <div className="max-w-2xl mx-auto antialiased pt-4 relative">
        {TermsAndConditionData.map((item: TermItem, index: number) => (
          <div key={`content-${index}`} className="mb-10">
            <h2 className="hgeading rounded-full text-sm w-fit px-4 py-1 mb-4">
              {item.badge}
            </h2>

            <h2 className="heading2 text-gradient mb-4 ">{item.heading}
            </h2>
